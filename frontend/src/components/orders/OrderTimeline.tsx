...
export default function OrderTimeline({ order, currentStatus = 'pending' }: OrderTimelineProps) {
  const currentIndex = statusSteps.findIndex(step => step.key === currentStatus);

  // If no order provided, show a placeholder
  if (!order) {
    return (
      <div className="flow-root p-8 text-center text-gray-500">
            ...
            <li key={step.key}>
              <div className="relative pb-8">
                {stepIdx !== statusSteps.length - 1 && (
                  <span
                    className={`absolute top-4 left-4 -ml-px h-full w-0.5 ${isCompleted ? 'bg-blue-600' : 'bg-gray-200'
                      }`}
                    aria-hidden="true"
                  />
                )}
                <div className="relative flex space-x-3">
                  <div>
                    <span
                      className={`h-8 w-8 rounded-full flex items-center justify-center ring-8 ring-white ${isCompleted || isCurrent ? 'bg-blue-600' : 'bg-gray-200'
                        }`}
                    >
                      {isCompleted ? (
                  ...
                  <div className="flex min-w-0 flex-1 justify-between space-x-4 pt-1.5">
                    <div>
                      <p
                        className={`text-sm font-medium ${isCompleted || isCurrent ? 'text-blue-600' : 'text-gray-500'
                          }`}
                      >
                        {step.label}
                      </p>
